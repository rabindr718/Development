...
function handleSubmit(event) {
  ...
  renderFormData();
  completed: false; // Added to track completion status
}
...
function renderFormData() {
  ...
  formDataArray.forEach((data) => {
    ...
    checkboxSpan.innerHTML = `<input type="checkbox" id="vehicle3" name="vehicle${
      data.id
    }" ${data.completed ? "checked" : ""} onchange="toggleComplete(${
      data.id
    })">`;
...
function toggleComplete(id) {
  const itemToToggle = formDataArray.find((item) => item.id === id);
  if (itemToToggle) {
